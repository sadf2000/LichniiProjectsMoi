...
int main() {
    int i = 0;
    printf("Kakui mi xotim vziat muziku(0 - Happy Birthday, 1 - chilovia musica)? \n");
    scanf("%d", &i); // Исправлено: scanf использует форматный спецификатор %d для int. &i
    ...
    if (i == 0) {
        ...
        generate_melody_wav("happy_birthday.wav", melody, sizeof(melody) / sizeof(Note)); // Changed filename
    } else if (i == 1) {
        Note melody[] = {
            // Нота, длительность (сек), громкость (0-1)
        ...
        generate_melody_wav("chilovaia_music.wav", melody, sizeof(melody) / sizeof(Note)); // Changed filename
    }
    else{
        printf("Oshibka\n");
    }
